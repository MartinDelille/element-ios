    ...
    init(parameters: PollEditFormViewModelParameters) {
        let state = PollEditFormViewState(
            ...
            bindings: PollEditFormViewStateBindings(
                question: EditFormPollQuestion(text: parameters.pollDetails.question, maxLength: Constants.maxQuestionLength),
                answerOptions: parameters.pollDetails.answerOptions.map { EditFormPollAnswerOption(text: $0, maxLength: Constants.maxAnswerOptionLength) },
    ...
    override class func reducer(state: inout PollEditFormViewState, action: PollEditFormStateAction) {
        ...
        case .viewAction(let viewAction):
            ...
            case .addAnswerOption:
                state.bindings.answerOptions.append(EditFormPollAnswerOption(text: "", maxLength: Constants.maxAnswerOptionLength))
            default:
                break
            }
        case .startLoading:
